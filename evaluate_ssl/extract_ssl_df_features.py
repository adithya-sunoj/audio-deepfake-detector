...
OUTPUT_CSV = os.path.join(BASE_DIR, "eval_df_ssl_features.csv")
...
# ---------------- 3. Main Execution (With Resume & RAM Safety) ----------------
def main():
    ...
    # To avoid overwriting old checkpoints, figure out the next checkpoint number
    checkpoint_num = len(ckpt_files) + 1

    dataset = ASVspoofDataset(labels_df, FLAC_DIR)
    loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=pad_collate, num_workers=0, pin_memory=True)
    
    feature_rows = []
    
    print(f"Resuming batched extraction (Batch Size: {BATCH_SIZE})...")
    with torch.no_grad():
